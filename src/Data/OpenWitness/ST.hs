...
import Data.Witness.Specific.WitnessOfDict
import Prelude

type ST s = StateT (WitnessOfDict (OpenWitness s)) (OW s)

stToOW :: ST s a -> OW s a
stToOW st = evalStateT st emptyWitnessOfDict
...
newSTRef a = do
    ...
    dict <- get
    put (witnessOfDictAdd wit a dict)
    return wit

readSTRef :: STRef s a -> ST s a
readSTRef key = do
    dict <- get
    case witnessOfDictLookup key dict of
        Just a -> return a
        _ -> error "ref not found"

writeSTRef :: forall s a. STRef s a -> a -> ST s ()
writeSTRef key newa = modify (witnessOfDictReplace key newa)

modifySTRef :: forall s a. STRef s a -> (a -> a) -> ST s ()
modifySTRef key amap = modify (witnessOfDictModify key amap)
